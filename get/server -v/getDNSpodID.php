<?php
	...
	class getDNSpodID
	{
		...
		private $login_token = '123456,efehfuiewhfuiwe34765984378679';
		...
		public function __construct()
		{
			...
			if($domain_list)
			{
				for($i=0; $i<count($domain_list); $i++)
				{
					...
					echo "域名ID(domain_id)：" . $domain_list[$i]["id"] . "<br>";
					echo "域名名称：" . $domain_list[$i]["name"] . "<br>";
					if($record_list <= 0)
					{
						echo "<br>" . "该域名无解析记录" . "<br>";
					}
					...
					{
						echo "<br>" . "记录ID(record_id)：" . $record_list[0]["id"] . "<br>";
						echo "域名前缀：" . $record_list[0]["name"] . "<br>";
						echo "解析地址：" . $record_list[0]["value"] . "<br>";
					}
					else
					{
						for($j=0; $j<count($record_list); $j++)
						{
							echo "<br>" . "记录ID(record_id)：" . $record_list[$j]["id"] . "<br>";
							echo "域名前缀：" . $record_list[$j]["name"] . "<br>";
							echo "解析地址：" . $record_list[$j]["value"] . "<br>";
						}
					}
