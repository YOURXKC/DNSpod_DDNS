<?php
	...
	*/ 
	...
	class setDNSpodIP
	{
		...
		//静态数据库变量（MySql）
		private $dbURL = 'localhost';//数据库地址
		private $dbPort = '3306';//数据库端口
		private $dbName = 'ddns_ip';//数据库名称
		private $dbTable = 'ip_table';//数据表名称
		private $dbUser = 'root';//数据库用户名
		private $dbPassword = 'root';//数据库密码
		...
		*/
		public function __construct()
		{
			...
			//3.判断IP是否更换
			$ip_data = $this->ifSaveIp($conn,$new_ip);
			if($ip_data["bool"])
			{//不更换
			...
			//5.更新本地数据库IP记录(程序结束)
			$up_bool = $this->setDbIp($conn,$new_ip);
			if($up_bool)
			{
				//打印日志（注意：命令行下导出日志路径必须为绝对路径）
				file_put_contents($this->log_path, '('.date('Y-m-d H:i:s',time()).')——[更新数据库IP记录成功]: 新->'.$new_ip.',旧->'.$ip_data["old_ip"]."\n", FILE_APPEND);
			}
			else
			{
				//打印日志（注意：命令行下导出日志路径必须为绝对路径）
				file_put_contents($this->log_path, '('.date('Y-m-d H:i:s',time()).')——[更新数据库IP记录失败]: 新->'.$new_ip.',旧->'.$ip_data["old_ip"]."\n", FILE_APPEND);
			}
		...
		*/  
		private function getPublicIp() 
		{
	 	...
	 	{
	 		//连接mysql数据库
	 		$dburl = $this->dbURL . ':' . $this->dbPort;
	 		$conn = mysqli_connect($dburl, $this->dbUser, $this->dbPassword, $this->dbName) or die('ERROR:连接错误'.mysqli_connect_error());
	 		return $conn;
	 	}
	 	...
	 	//判断IP是否已经更换
	 	private function ifSaveIp($conn,$new_ip)
	 	{
	 		//编写sql语句(判断IP是否已存在)
	 		...
	 		//判断是否存在（若已经存在该IP，则不更新；不存在，则更新记录）
	 		if($row["ip"] == $new_ip)
	 		{//存在
	 			return ["bool"=>true,"old_ip"=>$row["ip"]];
	 		}
	 		else
	 		{//不存在
	 			return ["bool"=>false,"old_ip"=>$row["ip"]];
	 		}
	 	}
		...
		*/ 
	 	...
	 	//更新本地数据库IP记录
	 	private function setDbIp($conn,$new_ip)
	 	{
 			//编写更新数据库的SQL
 			$sql = "UPDATE ". $this->dbTable ." SET ip='".$new_ip."' WHERE id=1";
 			//执行sql
 			$up_bool = mysqli_query($conn,$sql);
 			return $up_bool;
	 	}
